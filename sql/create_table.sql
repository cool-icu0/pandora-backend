...
-- 创建库
create database if not exists codepandora;
...
CREATE TABLE IF NOT EXISTS question_view (
                                             viewId INT AUTO_INCREMENT PRIMARY KEY COMMENT '浏览记录Id',
                                             userId INT NOT NULL COMMENT '访问用户Id',
                                             questionId INT NOT NULL COMMENT '被浏览的面试题的唯一标识',
                                             viewTimestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '用户访问该题目的时间戳',
                                             UNIQUE INDEX IDX_userId_questionId (userId, questionId)
    ) COMMENT='存储用户对面试题目的浏览记录';
